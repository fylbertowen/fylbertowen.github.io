...
// ...existing code...
// Leaderboard update setelah quiz selesai
function updateUserLeaderboard(score) {
  ...
  scoreCell.textContent = `Quiz Fisika — ${score * 100} XP`;
  rankCell.textContent = "#4";
  row.style.display = '';
}

// Modifikasi bagian akhir quiz fisika:
document.getElementById('quiz-next').onclick = function() {
  ...
  if (quizIndex < quizQuestions.length) {
    ...
    document.getElementById('quiz-area').innerHTML =
      `<h4>Quiz selesai!</h4><p>Skor akhir kamu: <b>${quizScore} / ${quizQuestions.length}</b></p>`;
...
function saveScoreToLeaderboard(nama, skor) {
...
function loadLeaderboard() {
  db.ref('leaderboard').orderByChild('skor').limitToLast(10).once('value', snapshot => {
    ...
    snapshot.forEach(child => {
      ...
      tr.innerHTML = `<td class="rank">#${rank++}</td>
        <td class="name"><span>🟡</span> ${data.nama}</td>
        <td>Quiz Fisika — ${data.skor * 100} XP</td>`;
      tbody.prepend(tr);
    });
  });
}
...
function saveScoreToLeaderboard(nama, skor) {
  db.ref('leaderboard').push({
    ...
    loadLeaderboard(); // Panggil ini agar leaderboard langsung update
  });
}
